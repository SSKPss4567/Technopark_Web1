...
from questions.models import Question, Answer, Tag, UserProfile, QuestionLike, AnswerLike
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        q_like_counts = QuestionLike.objects.values('question_id').annotate(count=Count('pk'))
        questions_to_update = []
        for entry in q_like_counts.iterator(chunk_size=batch_size):
            questions_to_update.append(Question(id=entry['question_id'], like_count=entry['count']))
        with transaction.atomic():
            for i in range(0, len(questions_to_update), batch_size):
                Question.objects.bulk_update(questions_to_update[i:i + batch_size], ['like_count'])
        ...
        for entry in ans_counts.iterator(chunk_size=batch_size):
            questions_to_update.append(Question(id=entry['question_id'], answer_count=entry['count']))
        with transaction.atomic():
            for i in range(0, len(questions_to_update), batch_size):
                Question.objects.bulk_update(questions_to_update[i:i + batch_size], ['answer_count'])

        # Обновляем like_count у ответов
        a_like_counts = AnswerLike.objects.values('answer_id').annotate(count=Count('pk'))
        answers_to_update = []
        for entry in a_like_counts.iterator(chunk_size=batch_size):
            answers_to_update.append(Answer(id=entry['answer_id'], like_count=entry['count']))
        with transaction.atomic():
            for i in range(0, len(answers_to_update), batch_size):
                Answer.objects.bulk_update(answers_to_update[i:i + batch_size], ['like_count'])

        self.stdout.write(self.style.SUCCESS("DB filled and counters updated successfully!"))
